...
const unfundedGames = GAMES_JSON.filter(game => game.pledged < game.goal);
const MAXRAISED = GAMES_JSON.reduce((acc, game) => acc + game.pledged, 0);
// create a string that explains the number of unfunded games using the ternary operator
const strDisplay = `A total of $${MAXRAISED.toLocaleString()} has been raised for ${GAMES_JSON.length} games. Currently, ${unfundedGames.length} ${unfundedGames.length === 1 ? "game remains" : "games remain"} unfunded. We need your help to fund these amazing games!`

// create a new DOM element containing the template string and append it to the description container
const displayElement = document.createElement("div");
displayElement.innerHTML = strDisplay;
